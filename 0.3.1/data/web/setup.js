...
// ------------------------------------------------------------ schede
const STEP_LABELS = {intro: 'Welcome', platforms: 'Platforms', twitch: 'Twitch', tiktok: 'TikTok', youtube: 'YouTube',
  kick: 'Kick', gioco: 'Game Capture', alerts: 'Alert Controls', done: 'Finish'};
...
function renderRail(list) {
  ...
  rail.innerHTML = '';
  list.forEach((step, i) => {
    const item = el('li', i < state.step ? 'is-done' : i === state.step ? 'is-now' : '');
    const dot = el('span', 'steps__dot');
    if (i < state.step) dot.append(icon('i-check'));
    else dot.textContent = String(i + 1);
    item.append(dot, el('span', 'steps__label', STEP_LABELS[step] || step));
    rail.append(item);
  });
  $('progress-bar').style.width = `${Math.round(state.step / Math.max(1, list.length - 1) * 100)}%`;
  $('progress-label').textContent = `Step ${state.step + 1} of ${list.length}`;
}
...
function renderStep(list) {
  ...
  hero.append(el('p', 'hero__sub', 'One chat for every stream, plus a dock that shows follows, subscriptions, and gifts. Connect your channels in under two minutes.'));
    ...
    const make = (iconName, tone, title, text) => {
      const card = el('div', 'feat');
      const box = el('span', `pick__icon ${tone}`);
      box.append(icon(iconName));
      const body = el('span');
      body.append(el('b', '', title), el('small', '', text));
      card.append(box, body);
      return card;
    };
    feats.append(make('i-chat', 'twitch', 'Crossfeed Chat', 'all your chats in one place, with moderation, emotes, and replies'));
    feats.append(make('i-bell', 'kick', 'Crossfeed Activity', 'followers, subscriptions, gifts, bits, and raids in real time'));
    hero.append(feats);
    view.append(hero);
  ...
  // ultima scheda: riepilogo
  const success = el('div', 'success');
  success.innerHTML = '<svg viewBox="0 0 52 52"><circle cx="26" cy="26" r="24"/><path d="M15 27l7 7 15-16"/></svg>';
  view.append(success);
  view.append(el('h1', '', 'All Set'));
  view.append(el('p', '', 'Crossfeed is configured. Both docks are in OBS; if you close them, reopen them from the Crossfeed menu, where you also find this guide again.'));
  const rows = el('div', 'rows');
  for (const [key, info] of Object.entries(PLATFORMS)) {
    if (!state.chosen[key]) continue;
    const row = el('div', 'row');
    row.append(platformIcon(key), el('b', '', info.name));
    const ok = key === 'twitch' ? !!(state.channels.twitch && state.account) : !!state.channels[key];
    const shown = (state.checks[key] && state.checks[key].display) || state.channels[key];
    const label = !state.channels[key] ? 'channel missing'
      : key === 'twitch' && !state.account ? 'read-only'
      : info.ready ? shown : shown + ' \u00b7 in development';
    row.append(el('span', `state ${ok ? 'ok' : 'ko'}`, label));
    rows.append(row);
  }
  view.append(rows);
  const game = el('div', 'box');
  game.append(el('h3', '', 'Automatic Game Capture'));
  game.append(el('span', 'state ' + (state.autoGame ? 'ok' : 'ko'),
                 state.autoGame ? 'enabled: open your game and Crossfeed handles it'
                                : 'disabled: choose the window to capture yourself'));
  view.append(game);
