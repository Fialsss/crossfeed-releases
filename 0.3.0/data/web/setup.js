...
function render() {
  ...
  list.forEach((_, i) => {
    const dot = el('span');
    if (i < state.step) dot.className = 'is-done';
    if (i === state.step) dot.className = 'is-now';
    dots.append(dot);
  });
  ...
  if (name === 'platforms') {
    ...
    for (const [key, info] of Object.entries(PLATFORMS)) {
      ...
      if (!info.ready) title.append(el('span', 'soon', 'presto'));
  ...
  view.append(el('p', '', 'Crossfeed is configured. Both docks are in OBS; if you close them, reopen them from the Docks menu.'));
  ...
  box.append(el('h3', '', 'Need It Again?'));
  box.append(el('p', '', 'Open this wizard again from OBS at any time: Crossfeed → Setup Wizard.'));
  view.append(box);
...
function checkedField(key, label, placeholder, help) {
  ...
  wrap.append(field(label, state.channels[key], placeholder, help, value => {
    ...
    result.innerHTML = '';
    if (!value) return;
    result.append(el('span', 'state', 'Looking up the profile...'));
...
function runCheck(key, result) {
  ...
  post('/api/check', {platform: key, name}).then(info => {
    ...
    result.append(el('small', 'checked__label', 'Profile found:'));
    result.append(profileCard(info, {key}));
  });
}
