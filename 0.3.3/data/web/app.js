...
// Who joins the TikTok LIVE: one line at the bottom of the chat that the next joiner replaces, like the
// chat in the TikTok app. Never a list: at most one change every 600 ms, gone 4 s after the last one.
let joinTimer = 0, joinHide = 0, joinLast = 0, joinNext = null;
function showJoin(m) {
  joinNext = m;
  if (joinTimer) return;
  joinTimer = setTimeout(() => {
    ...
    const next = joinNext;
    joinNext = null;
    const ticker = $('join-ticker');
    if (!next || !visible(next) || !$('resume').hidden) return;
    const who = document.createElement('b');
    who.textContent = next.name;
    ticker.replaceChildren(sourceIcon('tiktok'), who, document.createTextNode(' joined'));
    ticker.classList.remove('is-on');
    void ticker.offsetWidth; // the pill bumps for every new name
    ticker.classList.add('is-on');
    clearTimeout(joinHide);
    joinHide = setTimeout(() => ticker.classList.remove('is-on'), 4000);
  }, Math.max(0, 600 - (Date.now() - joinLast)));
}
