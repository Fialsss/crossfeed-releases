...
function renderNow() {
  ...
  for (const it of items) {
    ...
    // per le tinte: rgba(var(--row-rgb), a) invece di color-mix(), che il Chromium di OBS 30 (103) non ha
    li.style.setProperty('--row-rgb', [1, 3, 5].map(i => parseInt(color.slice(i, i + 2), 16)).join());
...
$('pref-demo').onclick = () => {
  ...
  const rows = [
    ...
    {platform: 'twitch', type: 'gift', name: 'Babbo', detail: 'to Fortunato · Tier 1', time: now - 30 * m},
