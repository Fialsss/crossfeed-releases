...
// stripe on the left. The chat message only says it comes from a reward (rewardId); title and cost arrive
// separately from EventSub (the "activity" event, the same one the Activity dock uses). They are matched by
// viewer, whichever arrives first. A reward without text becomes a line of its own.
const rewardLines = new Map();  // userId -> {line, at}: chat messages waiting for their reward
const rewardEvents = new Map(); // userId -> {event, timer}: rewards waiting for their chat message
function rewardHeader(event, withName) {
  const head = document.createElement('div');
  head.className = 'reward-line__head';
  if (withName) {
    const who = document.createElement('b');
    who.textContent = event.name;
    head.append(who, document.createTextNode(' redeemed '));
  } else {
    head.append(document.createTextNode('Redeemed '));
  }
  head.append(document.createTextNode(event.reward || 'a reward'));
  if (event.cost) {
    const cost = document.createElement('span');
    cost.className = 'reward-line__cost';
    cost.innerHTML = '<svg><use href="#i-points"/></svg>';
    cost.append(document.createTextNode(String(event.cost)));
    head.append(cost);
  }
  return head;
}
function decorateReward(line, event) {
  ...
  line.prepend(rewardHeader(event, false));
}
...
function onReward(event) {
  const key = event.userId || (event.login || '').toLowerCase();
  const waiting = rewardLines.get(key);
  if (waiting && Date.now() - waiting.at < 15000) {
    rewardLines.delete(key);
    decorateReward(waiting.line, event);
    return;
  }
  if (!event.input) return rewardLine(event); // nothing will come in chat for this one
  // the chat message is on its way: wait for it a few seconds, then show the reward on its own
  clearTimeout((rewardEvents.get(key) || {}).timer);
  rewardEvents.set(key, {event, timer: setTimeout(() => { rewardEvents.delete(key); rewardLine(event); }, 4000)});
}
...
function addMessage(m) {
  ...
  if (m.rewardId && m.platform === 'twitch') {
    const key = m.userId;
    const pending = rewardEvents.get(key);
    if (pending) {
      clearTimeout(pending.timer);
      rewardEvents.delete(key);
      decorateReward(line, pending.event);
    } else {
      rewardLines.set(key, {line, at: Date.now()});
      // no title from Twitch (for example without the rewards permission): still say it is a reward
      setTimeout(() => {
        if (rewardLines.get(key)?.line === line) { rewardLines.delete(key); decorateReward(line, {}); }
      }, 6000);
    }
  }
  const stick = !paused && atBottom();
...
function showDemo() {
  ...
  onReward({userId: 'd11', name: 'mi__chiamano__alfredo', reward: 'TTS', cost: 100, input: 'Pronto, centralino?'});
  addMessage({platform: 'twitch', id: 'd11', userId: 'd11', name: 'mi__chiamano__alfredo', color: '#00a8a8', text: 'Pronto, centralino?', badges: [], emotes: [], time: now, rewardId: 'demo'});
  onReward({userId: 'd12', name: 'Luna', reward: 'Hydrate!', cost: 500, input: ''});
